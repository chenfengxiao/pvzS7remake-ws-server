...
export function startHomeMqttTunnel(options = {}) {
  ...
  function publishStatus(online) {
    ...
    const body = JSON.stringify({ v: 1, online: !!online, expiresAt: Date.now() + (online ? STATUS_TTL_MS : 1000), serverVersion: options.serverVersion || '1.7.1' });
  ...
  mqtt.onmessage = (topic, payload) => {
    ...
    if (obj && obj.__tunnel === 'hello') {
      ...
      bridge._publishControl(sock, { __tunnel: 'welcome', serverVersion: options.serverVersion || '1.7.1', serverTime: Date.now() });
      return;
    }
    if (!sock || sock.readyState !== 1) return;
