...
function autoChoice(side,kind){const d=P.draft;const own=d.pool[side].filter(x=>!unavailable(x));if(!own.length)return null;if(window.S7VersusAI?.draftChoice)return window.S7VersusAI.draftChoice(side,kind,d.pool[side],id=>unavailable(id),{picks:d.picks,bans:d.bans,step:d.i});return own[(d.i*7+(side==='plant'?3:1))%own.length]}
