...
function buildSteps(){const picks=P.slots-1,steps=[];if(P.bp){for(let i=0;i<5;i++){steps.push({side:'zombie',kind:'ban'});steps.push({side:'plant',kind:'ban'})}}for(let i=0;i<picks;i++){steps.push({side:'zombie',kind:'pick'});steps.push({side:'plant',kind:'pick'})}return steps}
...
function advanceAi(){const st=current();if(!st){finishDraft();return}if(st.side===P.side)return;setTimeout(()=>{const targetSide=st.kind==='ban'?(st.side==='zombie'?'plant':'zombie'):st.side;const id=autoChoice(targetSide,st.kind);if(id)act(id);else{P.draft.i++;renderDraft();advanceAi()}},140)}
...
function canvasPointer(ev){const B=window.S7VersusBattle?.state;if(!B?.active||B.mode!=='practice')return;const rect=canvas.getBoundingClientRect();const side=P.side;if(side==='plant'&&B.shovelMode){const h=(()=>{const x=(ev.clientX-rect.left)/rect.width,y=(ev.clientY-rect.top)/rect.height;return {row:Math.max(0,Math.min(4,Math.floor((y-(layout.y/innerHeight))/(layout.cell/innerHeight)))),col:Math.max(0,Math.min(8,Math.floor((x-(layout.x/innerWidth))/(layout.cell/innerWidth))))}})();window.S7VersusBattle.performAction({type:'shovel',side:'plant',row:h.row,col:h.col});B.shovelMode=false;return}const a=window.S7VersusBattle.actionFromPointer(side,ev.clientX,ev.clientY,rect);window.S7VersusBattle.performAction(a)}
