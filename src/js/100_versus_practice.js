...
function aiTick(){const B=window.S7VersusBattle?.state;if(!B?.active||B.mode!=='practice'||B.versus?.phase!=='battle')return;const ai=P.side==='plant'?'zombie':'plant';if(state.time-P.lastAiAt<1.0)return;P.lastAiAt=state.time;const list=window.S7VersusBattle.cardsFor(ai).slice(1);if(ai==='plant'){
