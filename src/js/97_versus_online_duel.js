...
function onBattleStart(m){const my=O.playerId,host=m.hostId===my,side=m.sides?.plant===my?'plant':'zombie';O.side=side;hide('versusOnlineDraftScreen');if(host){document.getElementById('game')?.classList.remove('hidden');document.getElementById('startScreen')?.classList.add('hidden');try{window.s7SetBattleSeed?.(m.seed)}catch(_){}window.S7VersusBattle.start({mode:'online',online:true,isHost:true,humanSide:side,role:side,room:O.room,plantCards:m.picks.plant,zombieCards:m.picks.zombie});window.S7VersusRealtime?.startHost?.(side)}else{show('versusRealtimeScreen');window.S7VersusRealtime?.startGuest?.(side)}}
