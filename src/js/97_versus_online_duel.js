...
function draftPhases(v){const picks=(v.slots||6)-1;if(v.bp&&window.S7VersusBP?.buildPhases)return window.S7VersusBP.buildPhases((v.slots||6)===7).map(ph=>({side:ph.actor,kind:ph.action,target:ph.targetSide}));const out=[];for(let i=0;i<picks;i++){out.push({side:'zombie',kind:'pick',target:'zombie'},{side:'plant',kind:'pick',target:'plant'})}return out}function draftStep(v){const phases=draftPhases(v),step=v.step||0,total=phases.length;if(step>=total)return {done:true,total};return {...phases[step],total}}
function used(v,id){return [...(v.picks?.plant||[]),...(v.picks?.zombie||[]),...(v.bans?.plant||[]),...(v.bans?.zombie||[])].includes(id)}
function renderDraft(){const r=O.room,v=r?.versus;if(!r||!v)return;hide('roomScreen');show('versusOnlineDraftScreen');const st=draftStep(v);const set=(id,fn)=>{const el=document.getElementById(id);if(el)fn(el)};set('versusDraftProgress',el=>el.textContent=`${v.step||0} / ${st.total||0}`);set('versusDraftRuleBadge',el=>el.textContent=`${v.slots||6}槽 · ${v.bp?'有BP':'No-BP'} · 僵尸先手`);set('versusDraftZombieName',el=>el.textContent=nick(v.sides.zombie));set('versusDraftPlantName',el=>el.textContent=nick(v.sides.plant));const chip=(side)=>(v.picks?.[side]||[]).map(id=>`<span class="versusPickChip">${window.S7VersusBattle.cardName(side,id)}</span>`).join('')||'<span class="versusPickChip">待选择</span>';set('versusZombiePickSlots',el=>el.innerHTML=chip('zombie'));set('versusPlantPickSlots',el=>el.innerHTML=chip('plant'));set('versusZombieBanLine',el=>el.textContent='被Ban：'+((v.bans?.zombie||[]).map(id=>window.S7VersusBattle.cardName('zombie',id)).join('、')||'无'));set('versusPlantBanLine',el=>el.textContent='被Ban：'+((v.bans?.plant||[]).map(id=>window.S7VersusBattle.cardName('plant',id)).join('、')||'无'));if(st.done){set('versusDraftTurnBanner',el=>el.innerHTML='<small>选卡完成</small><strong>双方卡组已锁定</strong><span>等待房主开始</span>');set('versusDraftInstruction',el=>el.textContent='选卡完成');set('versusOnlineCardGrid',el=>el.innerHTML='');set('versusDraftStartBattleBtn',el=>el.classList.toggle('hidden',!O.host));return}const mine=v.sides?.[st.side]===O.playerId;set('versusDraftTurnBanner',el=>el.innerHTML=`<small>${st.kind==='ban'?'BAN':'PICK'}</small><strong>${st.side==='zombie'?'🧟 僵尸方':'🌱 植物方'}${mine?'（你）':''}</strong><span>${mine?'轮到你操作':'等待对方'}</span>`);set('versusDraftInstruction',el=>el.textContent=mine?`轮到你 ${st.kind==='ban'?'Ban 对方1张卡':'Pick 1张自己的卡'}`:`等待${st.side==='zombie'?'僵尸':'植物'}方操作`);const ids=Object.keys(window.S7VersusBattle.CARDS[st.target]||{});set('versusOnlineCardGrid',el=>{el.innerHTML=ids.map(id=>{const c=window.S7VersusBattle.cfg(st.target,id),dis=used(v,id);return `<button class="versusDraftCard ${dis?'disabled':''}" data-id="${id}"><b>${window.S7VersusBattle.cardName(st.target,id)}</b><span>${c.cost} ${st.target==='plant'?'阳光':'脑光'}</span><small>CD ${c.cd}s${c.guaranteed?` · 100%变种 ${c.guaranteed}`:''}</small></button>`}).join('')});set('versusOnlineCardGrid',el=>el.querySelectorAll('.versusDraftCard').forEach(b=>b.onclick=()=>{if(mine&&!used(v,b.dataset.id))send({type:'versusDraftAction',cardId:b.dataset.id})}));set('versusDraftStartBattleBtn',el=>el.classList.add('hidden'))}
function onBattleStart(m){const my=O.playerId,host=m.hostId===my,side=m.sides?.plant===my?'plant':'zombie';O.side=side;hide('versusOnlineDraftScreen');hide('versusRealtimeScreen');hide('roomScreen');document.getElementById('game')?.classList.remove('hidden');document.getElementById('startScreen')?.classList.add('hidden');try{window.s7SetBattleSeed?.(m.seed)}catch(_){}window.S7VersusBattle.start({mode:'online',online:true,isHost:!!host,humanSide:side,role:side,room:O.room,plantCards:m.picks.plant,zombieCards:m.picks.zombie});window.S7VersusRealtime?.[host?'startHost':'startGuest']?.(side)}
...
function handle(msg){
  ...
  if(msg.type==='versusRoomCreated'||msg.type==='versusRoomJoined'||(msg.type==='roomResumed'&&msg.room?.kind==='versus')){
    ...
    if(msg.room?.state==='battling'){
      ...
      window.S7VersusRealtime?.[msg.room.hostId===O.playerId?'startHost':'startGuest']?.(side)
    }
    return;
  }
