...
(function registerVersusAssets(){
 try{
  ...
  S7_SPRITES.register("versus.target.victory","./assets/final_runtime/target_zombie_victory.png",{frameWidth:128,frameHeight:128,columns:8,frameCount:26});
...
function productionTick(dt){
 ...
 for(const z of state.zombies){if(!z.dead&&z.versusStatic==="grave"){
  const upPeriod=z.versusUpgraded?R.twinProductionPeriodSeconds*.75:R.twinProductionPeriodSeconds;
  if(!z.versusEconomy)z.versusEconomy={nextProduceAt:state.time+upPeriod,phase:"idle",phaseStartedAt:0,pendingAmount:0};
  ...
  if(z.versusUpgraded&&!B.versus.suddenDeath){
   ...
   if(z.versusBlindBoxTimer>=R.blindBoxSummonInterval){z.versusBlindBoxTimer=0;const nz=makeZombie("blind",z.row,null,{variant:false});nz.x=z.x;if(nz.baseX!=null)nz.baseX=nz.x;state.zombies.push(nz)}
  }
 }}
...
function deployZombie(id,row,guaranteed=false,deployX){
 if(row<0||row>=5)return {ok:false,reason:"路线无效"};if(!ZOMBIES[id]&&id!=="blind")return {ok:false,reason:"僵尸不存在"};
...
function handleTargetDeath(z){if(!isVersusTarget(z))return;if(z._targetCounted)return;z._targetCounted=true;z._targetDiedAt=state.time;B.versus.target.destroyed++}
...
function tick(dtOverride){
 ...
 // 更新僵尸血量倍率显示
 try{const e=document.getElementById('versusHpMultBadge');if(e){const mult=timeHpMult(state.time);const t=e.querySelector('b');if(t)t.textContent=mult.toFixed(3).replace(/\.?0+$/,'')+'×';e.classList.remove('hidden');e.style.display='';}}
 catch(_){}
}
setInterval(tick,200);
function cardsFor(side){return [side==="plant"?FIXED.plant:FIXED.zombie].concat(side==="plant"?B.plantCards:B.zombieCards).concat(side==="zombie"?[FIXED.mystery]:[])}
...
function packetIcon(side,id){if(id===FIXED.plant)return "🌻🌻";if(id===FIXED.zombie)return "🪦";if(id===FIXED.mystery)return "❓🪦";if(id==="blind")return "🎁";if(ASH_VISUALS[id])return ASH_VISUALS[id].emoji;try{return (side==="plant"?PLANTS[id]?.emoji:ZOMBIES[id]?.emoji)||(side==="plant"?"🌿":"🧟")}catch(_){return side==="plant"?"🌿":"🧟"}}
function shortName(side,id){if(id===FIXED.plant)return "向";if(id===FIXED.zombie)return "碑";if(id===FIXED.mystery)return "召";if(id==="blind")return "盲";if(ASH_VISUALS[id])return ASH_VISUALS[id].emoji;try{return (side==="plant"?PLANTS[id]?.name:ZOMBIES[id]?.name)?.slice(0,1)||id}catch(_){return id}}
