...
function summarize(matches, side){
  ...
  const wastedAsh = ash.filter(d => d.resolvedPaidValueDamageEquivalent < d.paidCost * 0.5);
  return {
    ...
    ashAvgResolved: ash.length ? Math.round(ash.reduce((s, d) => s + d.resolvedPaidValueDamageEquivalent, 0) / ash.length) : 0,
